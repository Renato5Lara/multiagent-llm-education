...
export function DecisionMenu({ menu, mastery, onChoose }: Props) {
  ...
  return (
    <div className="glass-panel rounded-2xl p-6 space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <p className="text-sm font-medium text-neural-text leading-snug">{menu.question}</p>
      </div>
      ...
      <div className="space-y-2">
        {menu.reinforcements.map(r => (
          <button
            ...
            className="w-full text-left px-4 py-3 rounded-xl border border-white/[0.08] bg-white/[0.02] text-sm text-neural-muted hover:border-neural-violet/40 hover:bg-neural-violet/5 hover:text-neural-text transition-colors"
          >
            {r.label}
          </button>
        ))}
