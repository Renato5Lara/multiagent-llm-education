...
import { useSubmitDiagnostic, useAgentGeneratePlan } from '@/hooks/useStudent'
...
export default function DiagnosticTest() {
    ...
    const generatePlan = useAgentGeneratePlan()
    ...
    const handleNext = async () => {
        if (current < DIAGNOSTIC_QUESTIONS.length - 1) {
            ...
            setCompleted(true)
            if (courseId) {
                await submitDiagnostic.mutateAsync({ courseId, answers })
                await generatePlan.mutateAsync({ courseId, answers })
            }
        }
    }

    if (completed) {
        const isLoading = submitDiagnostic.isPending || generatePlan.isPending
        return (
                    ...
                    <CardContent className="p-8">
                        {isLoading ? (
                            <>
                                ...
                                <p className="text-muted-foreground">El sistema multiagente está analizando tus respuestas y generando tu ruta personalizada.</p>
                            </>
                        ) : (
                            <>
                                <CheckCircle2 className="h-16 w-16 text-green-500 mx-auto mb-4" />
                                <h2 className="text-xl font-bold mb-2">¡Test completado!</h2>
                                <p className="text-muted-foreground mb-6">Tu perfil de aprendizaje ha sido registrado. El sistema generará tu ruta personalizada.</p>
