"""Qué implementación de Diagnosticar corre en las invocaciones reales
...
Engine ni el grafo"; ADR-0007 clasifica Diagnosticar como CLAIM: el LLM
sí puede ser la fuente del veredicto, declarando la regla de dominio en
el prompt).
...
`runtime/engine/graph/walkthrough.py` NO cambia su valor por defecto —
sigue siendo la regla, para que la suite determinista (P12) no dependa
de red. Esta selección vive en el Boundary porque es quien construye
cada invocación real de `ejecutar_walkthrough`; los tests que exigen una
implementación concreta (P13, guardianes ADR-0005 §7) siguen pasándola
explícitamente, sin pasar por aquí.
"""
