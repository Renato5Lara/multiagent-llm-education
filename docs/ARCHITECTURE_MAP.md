...
> **⚠️ DESACTUALIZADO (2026-08-01):** Este documento describe `app.agents.*`
> (`BaseAgent`) y `app.swarm.*` (`SwarmOrchestrator`, `AgentFactory`) como
> arquitectura "V2 Current" — ambos fueron eliminados físicamente en
> `ADR-0011-retiro-fisico-baseagent-swarm-legacy.md` (ver
> `docs/architecture/ADR/`). La arquitectura multiagente vigente es
> `backend/runtime/` (LangGraph), trazada endpoint por endpoint en
> `docs/SWARM_ACTIVATION_AUDIT.md`. No usar este documento como fuente de
> verdad para `app/agents/`, `app/swarm/`, ni para el pipeline de 7
> agentes de `pedagogical_orchestration_service.py` (también eliminado).
