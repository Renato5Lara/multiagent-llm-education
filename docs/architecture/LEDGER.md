...
| D-045 | Calibración de confianza declarada para Remediar y Orientar (camino LLM): `evidence_strength` (Wilson, matemática pura, sin conocimiento de dominio) se relocaliza a `runtime/domain/shared/calibracion.py` y se reutiliza directo sobre la evidencia propia de cada productor (`errores`/`total` para Remediar, `aciertos`/`total` para Orientar); `calibrar_confianza_nueva` NO se reutiliza — su semántica de mejora/retroceso es específica del modelo temporal de Diagnosticar (claim vigente del mismo asunto), no aplica a una tensión D2 resuelta una sola vez por `mecanica.convocar()`. Alcance limitado a `productor_llm.py`; el camino de regla no se toca (mismo patrón que Diagnosticar-regla, confianza fija por convención) | ADR-0013 | Vigente |
